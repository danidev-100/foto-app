...
export default function Catalog({ onCartUpdate }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.log('Courses:', raw.map(c => ({ id: c.id, name: c.name })));
      console.log('Booklets:', (bookletsData.data || []).map(b => ({ id: b.id, course_id: b.course_id, title: b.title })));
    } catch {
  ...
  const handleSelectCourse = (course) => {
    console.log('Selected course:', course.name, '| ID:', course.id);
    console.log('Looking for booklets with course_id:', course.id);
    console.log('Available booklet course_ids:', [...new Set(allBooklets.map(b => b.course_id))]);
                  ...
                  >
                    ...
                    <div className="p-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      {levelCourses.map((course) => {
                        const courseBooklets = allBooklets.filter(b => b.course_id === course.id && b.is_active);
                        return (
                          <div
                            key={course.id}
                            className="card overflow-hidden ring-1 ring-white/80 bg-white"
                          >
                            {/* Course header */}
                            ...
                            <div className="p-4">
                              ...
                              {course.description && (
                                <p className="mt-1 text-xs text-surface-500 line-clamp-1">{course.description}</p>
                              )}

                              {/* Booklets list */}
                              {courseBooklets.length > 0 ? (
                                <div className="mt-3 space-y-2">
                                  {courseBooklets.map((b) => (
                                    <div key={b.id} className="flex items-center justify-between p-2 bg-surface-50 rounded-lg">
                                      <div className="min-w-0 flex-1">
                                        <p className="text-sm font-medium text-surface-900 truncate">{b.title}</p>
                                        {getDivisionsFromDesc(b.description) && (
                                          <span className="text-xs text-surface-500">Div: {getDivisionsFromDesc(b.description)}</span>
                                        )}
                                      </div>
                                      <div className="flex items-center gap-2 ml-2 shrink-0">
                                        <span className="text-sm font-bold text-primary-600">{formatPrice(b.current_price)}</span>
                                        <button
                                          onClick={() => handleAdd(b)}
                                          className="btn-primary inline-flex items-center gap-1 text-xs px-2 py-1"
                                        >
                                ...
                                <p className="mt-2 text-xs text-surface-400 italic">Sin cuadernillos</p>
                              )}
                            </div>
                          </div>
                        );
